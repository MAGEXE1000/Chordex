...
## Modern Migration (Firebase v12.16.0+)

In accordance with Firebase v12+ deprecation of `enableMultiTabIndexedDbPersistence()`, persistence is now configured directly during `initializeFirestore()` via:
...
`waitForFirestoreReady()` continues to resolve immediately after `initializeFirestore()` completes, ensuring full backward and forward compatibility without race conditions or deprecation warnings.
